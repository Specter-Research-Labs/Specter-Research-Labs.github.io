...
sync_follow_up_figures() {
    local md="blog/wonton-soup-follow-up/index.md"
    local assets_dir="assets/blog/wonton-soup-follow-up"
    local pdf_dir="${repo_root}/dossiers/wonton-soup/docs/figures/out"
    local converter=""
    ...
    mapfile -t png_names < <(
        grep -oE 'assets/blog/wonton-soup-follow-up/[A-Za-z0-9._-]+\.png' "$md" \
            | sed -E 's#^.*/##' \
            | sort -u
    )

    if [ "${#png_names[@]}" -eq 0 ]; then
        echo "No follow-up figure PNG references found in $md; skipping sync"
        return 0
    fi
...
is_published_post() {
    ...
    awk '
        ...
        /^---[[:space:]]*$/ { exit }
        { print }
    ' "$md" | grep -Eq '^[[:space:]]*publish:[[:space:]]*true[[:space:]]*$'
}
...
for md in "${posts[@]}"; do
    ...
    if ! is_published_post "$md"; then
        if [ -f "$out" ]; then
            rm -f "$out"
            echo "Removed stale draft output $out"
        fi
        echo "Skipped draft $md (set 'publish: true' in YAML front matter to publish)"
        continue
    fi
...
if [ "$published_count" -eq 0 ]; then
    echo "No published blog posts found (set 'publish: true' in YAML front matter)." >&2
    exit 1
fi
